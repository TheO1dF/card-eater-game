...
test("商店道具包含可负担的基础池，核心道具提价并延后解锁", () => {
  ...
  assert.ok(basic.every((entry) => entry.shop_price >= 2 && entry.shop_price <= 4));
  const reshuffleItem = pool.find((entry) => entry.id === "IT115");
  const membership = pool.find((entry) => entry.id === "IT113");
  assert.deepEqual([reshuffleItem.shop_price, reshuffleItem.min_shop_round], [9, 4]);
  assert.deepEqual([membership.shop_price, membership.min_shop_round], [10, 5]);
});

test("基础道具分别读取首次行动、行动位次与连续正确分类", () => {
  const state = readyState();
  const engine = createRoundEngine();
  addItem(state, "IT117");
  addItem(state, "IT118");
  ...
  const second = engine.recordAction(state, "discard", instance("A001", "basic-2"));
  const third = engine.recordAction(state, "eat", instance("F001", "basic-3"));
  assert.equal(first.item_bonus, 1);
  assert.equal(second.item_bonus, 1);
  assert.equal(third.item_bonus, 4);
});
...
test("欢迎页明确目标，规则选择显示进度，删牌必须二次确认", () => {
  ...
  assert.match(html, /第 5 \/ 10 \/ 15 轮累计达到 100 \/ 500 \/ 1800 分/);
...
test("真实商店决策可完成 15 轮，三类构筑批量通关率不低于 45%", () => {
  ...
  const reshuffleRun = simulateRun({ seed: 4, policy: "small" });
