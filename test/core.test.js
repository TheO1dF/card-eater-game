...
import { getRuleUnlockRound, isRuleEligible, randomDraftRules, RULE_LIBRARY } from "../js/rules.js";
...
test("除无类别虚空诅咒外，所有卡牌都有可执行效果", () => {
  ...
  assert.deepEqual([voidCard.type, voidCard.eat_points, voidCard.discard_points], ["无类别", -2, -1]);
});
...
test("基础金币按实体牌每轮首次吃计数，重洗不重复印钱", () => {
  const state = readyState();
  const engine = createRoundEngine();
  const repeated = instance("F001", "gold-repeat");
  engine.recordAction(state, "eat", repeated);
  engine.recordAction(state, "eat", repeated);
  engine.recordAction(state, "eat", instance("F001", "gold-other"));
  assert.equal(state.round.eat_sequence.length, 3);
...
test("阶段目标使用此前各轮累计总分，而不是只看目标轮得分", () => {
  ...
  state.total_score = 119;
  engine.recordAction(state, "eat", instance("F001", "milestone"));
  const result = engine.finalizeRound(state);
  assert.equal(result.round_score, 2);
  assert.equal(state.total_score, 121);
  assert.deepEqual(engine.levelProgressCheck(state), { passed: true, target: 120 });
});
...
test("收银台摧毁自身后提供两次免费刷新", () => {
  const state = readyState();
  const engine = createRoundEngine();
  const register = instance("U007", "double-reroll");
  state.deck.push(register);
  engine.recordAction(state, "discard", register);
  assert.equal(state.deck.some((card) => card.uuid === register.uuid), false);
  assert.equal(state.round.shop_free_rerolls, 2);

  const shop = createShopService({ random: () => 0.4, create_id: ids });
  assert.equal(shop.rerollShop(state).free, true);
  assert.equal(shop.rerollShop(state).free, true);
  assert.equal(state.round.shop_free_rerolls, 0);
  assert.equal(shop.getRerollCost(state), 3);
});
...
test("新增关键字、负牌面与商店折扣道具按不同资源轴生效", () => {
  ...
  assert.equal(state.round.pending_gold_bonus, 0);
  const shop = createShopService({ random: () => 0.5, create_id: ids });
...
test("规则池至少 40 条且同一局抽取绝不重复", () => {
  const state = readyState();
  assert.equal(RULE_LIBRARY.length, 70);
  assert.equal(new Set(RULE_LIBRARY.map((rule) => rule.id)).size, RULE_LIBRARY.length);
  assert.ok(RULE_LIBRARY.every((rule) => !rule.requires_reshuffle));
  assert.ok(RULE_LIBRARY.every((rule) => !["min_reshuffles", "repeat_card_actions", "post_reshuffle_actions", "post_reshuffle_score"].includes(rule.scope)));
...
test("规则按强度手工分层，首轮不出现容易达成的高倍率规则", () => {
  const state = readyState();
  const cleanPlate = RULE_LIBRARY.find((rule) => rule.id === "clean-plate");
  const tinyDeck = RULE_LIBRARY.find((rule) => rule.id === "tiny-deck-8");
  const sacrifice = RULE_LIBRARY.find((rule) => rule.id === "sacrifice-2");
  assert.equal(getRuleUnlockRound(cleanPlate), 3);
  assert.equal(getRuleUnlockRound(tinyDeck), 4);
  assert.equal(getRuleUnlockRound(sacrifice), 4);
  assert.equal(isRuleEligible(cleanPlate, state.deck, 1), false);
  assert.equal(isRuleEligible(cleanPlate, state.deck, 3), true);
  assert.ok(randomDraftRules(20, [], () => 0, state.deck, 1).every((rule) => getRuleUnlockRound(rule) === 1));
});
...
test("任务按行动施加明确惩罚，永久虚空牌吃弃都为负分", () => {
  ...
  assert.equal(engine.recordAction(voidState, "eat", voidCard).points, -2);
...
test("重启按钮与优惠打印机在每轮初始化为小牌组重洗和免费刷新", () => {
  const state = readyState();
  assert.equal(ITEM_LIBRARY.length, 36);
  assert.equal(createShopItemPool().length, 24);
...
test("商店道具包含可负担的基础池，核心道具按回报周期限时出现", () => {
  const pool = createShopItemPool();
  const basic = pool.filter((entry) => (entry.min_shop_round ?? 1) === 1 && entry.shop_price <= 6);
  assert.ok(basic.length >= 7);
  const reshuffleItem = pool.find((entry) => entry.id === "IT115");
  const membership = pool.find((entry) => entry.id === "IT113");
  assert.deepEqual([reshuffleItem.shop_price, reshuffleItem.min_shop_round, reshuffleItem.max_shop_round], [16, 7, 13]);
  assert.deepEqual([membership.shop_price, membership.min_shop_round, membership.max_shop_round], [9, 4, 9]);

  const investments = ["IT101", "IT103", "IT105", "IT106", "IT109", "IT110", "IT112", "IT113", "IT114", "IT116"]
    .map((id) => pool.find((entry) => entry.id === id));
  assert.ok(investments.every((entry) => Number.isInteger(entry.max_shop_round)));
  const lateState = readyState();
  lateState.current_round = 11;
  const lateOffers = createShopService({ random: () => 0, create_id: ids }).getShopItems(lateState);
  assert.ok(lateOffers.every((entry) => (entry.max_shop_round ?? 15) >= 11));
});

test("旧罗盘、候补餐罩和保温软垫不再被同价道具完爆", () => {
  const pool = createShopItemPool();
  assert.equal(pool.find((entry) => entry.id === "IT102").effect.kind, "first_correct_buff_next");
  assert.equal(pool.find((entry) => entry.id === "IT114").effect.kind, "reserve_purchase_refund");
  assert.equal(pool.find((entry) => entry.id === "IT121").effect.kind, "reserve_threshold_multiplier");

  const state = readyState();
  const engine = createRoundEngine();
  addItem(state, "IT102");
  engine.recordAction(state, "eat", instance("F001", "compass-first"));
  assert.equal(engine.recordAction(state, "eat", instance("F002", "compass-next")).item_bonus, 0);
  assert.equal(state.round.actions.at(-1).buff_flat_bonus, 2);

  const reserveState = readyState();
  reserveState.gold = 20;
  reserveState.round.reserve_count = 4;
  addItem(reserveState, "IT114");
  const shop = createShopService({ random: () => 0.5, create_id: ids });
  assert.equal(shop.buyCard(reserveState, { ...getCardById("F003"), shop_price: 3 }), true);
  assert.equal(reserveState.gold, 19);
  assert.equal(reserveState.last_shop_transaction.refund, 2);
  assert.equal(shop.buyCard(reserveState, { ...getCardById("F004"), shop_price: 3 }), true);
  assert.equal(reserveState.gold, 16);

  addItem(reserveState, "IT121");
  engine.recordAction(reserveState, "eat", instance("F001", "warm-pad"));
  assert.equal(engine.finalizeRound(reserveState).total_multiplier, 1.08);
});

test("经济道具分别兑现生成分、成长折扣、风险刷新和候补金币", () => {
  const state = readyState();
  const engine = createRoundEngine();
  addItem(state, "IT109");
  addItem(state, "IT110");
  addItem(state, "IT112");
  addItem(state, "IT116");
  state.round.reserve_count = 4;
  const generated = { ...instance("A010", "generated-item"), generated_from: "A009" };
  assert.equal(engine.recordAction(state, "eat", generated).item_bonus, 1);
  assert.equal(state.round.pending_gold_bonus, 1);
  engine.recordAction(state, "eat", instance("F005", "growth-discount"));
  assert.equal(state.round.shop_discount, 1);
  engine.recordAction(state, "eat", instance("F006", "risk-reroll"));
  assert.equal(state.round.shop_free_rerolls, 1);
  engine.recordAction(state, "discard", instance("A001", "reserve-gold"));
  assert.equal(state.round.pending_gold_bonus, 3);
});

test("24 件商店道具使用独立的新像素图，并移除失败的精确位次模板", () => {
  const pool = createShopItemPool();
  const spriteKeys = pool.map((entry) => `${entry.icon_atlas}:${entry.icon_x}:${entry.icon_y}`);
  const failedKinds = new Set([
    "exact_action_number_bonus",
    "middle_action_bonus",
    "correct_streak_bonus",
    "repeat_type_action_bonus",
    "different_type_same_action_bonus",
  ]);
  assert.equal(new Set(spriteKeys).size, 24);
  assert.equal(new Set(pool.map((entry) => entry.effect.kind)).size, 24);
  assert.ok(pool.every((entry) => entry.icon_atlas === "shop-items-atlas-v013.webp"));
  assert.ok(pool.every((entry) => !failedKinds.has(entry.effect.kind)));
  assert.ok(statSync(new URL("../assets/shop-items-atlas-v013.webp", import.meta.url)).size > 10_000);
});

test("基础道具分别支持小牌组双启动、反食性与单类别构筑", () => {
  const state = readyState();
  const engine = createRoundEngine();
  addItem(state, "IT117");
  addItem(state, "IT118");
  addItem(state, "IT119");
  const first = engine.recordAction(state, "eat", instance("F001", "basic-1"));
  const second = engine.recordAction(state, "discard", instance("F001", "basic-2"));
  const third = engine.recordAction(state, "eat", instance("A001", "basic-3"));
  const fourth = engine.recordAction(state, "discard", instance("A001", "basic-4"));
  assert.equal(first.item_bonus, 2);
  assert.equal(second.item_bonus, 4);
  assert.equal(third.item_bonus, 0);
  assert.equal(fourth.buff_flat_bonus, 2);

  const singletonState = readyState();
  const singleton = instance("A001", "singleton");
  singletonState.deck = [singleton, instance("F001", "other-type")];
  addItem(singletonState, "IT122");
  assert.equal(engine.recordAction(singletonState, "discard", singleton).item_bonus, 3);
});

test("候补餐签按餐盘外类别触发，且同类别每轮只计一次", () => {
  const state = readyState();
  const engine = createRoundEngine();
  addItem(state, "IT104");
  state.round.reserve_count = 2;
  state.round.reserve_type_counts = { 水果: 1, 星体: 1 };
  assert.equal(engine.recordAction(state, "eat", instance("F001", "reserve-a")).item_bonus, 1);
  assert.equal(engine.recordAction(state, "eat", instance("F001", "reserve-b")).item_bonus, 0);
});
...
test("欢迎页明确目标，规则选择显示进度，删牌必须二次确认", () => {
  const html = readFileSync(new URL("../index.html", import.meta.url), "utf8");
  const ui = readFileSync(new URL("../js/ui.js", import.meta.url), "utf8");
  assert.match(html, /明确目标/);
  assert.match(html, /第 5 \/ 10 \/ 15 轮累计达到 120 \/ 750 \/ 4000 分/);
  assert.match(html, /id="draftTargetProgress"/);
  assert.match(html, /id="deleteConfirm"[^>]*role="alertdialog"/);
  assert.match(html, /id="ruleInfoButton"/);
  assert.match(html, /id="itemInfoButton"/);
  assert.match(html, /id="ruleStatus"[^>]*role="dialog"/);
  assert.match(html, /id="itemStatus"[^>]*role="dialog"/);
  assert.match(ui, /openDeleteConfirmation/);
  assert.match(ui, /openRuleStatus/);
  assert.match(ui, /openItemStatus/);
  assert.match(ui, /确认支付.*金币删除/);
});

test("卡图不再绘制餐盘，罕见卡纸张不再偏绿", () => {
  const css = readFileSync(new URL("../styles.css", import.meta.url), "utf8");
  const cardArt = css.match(/\.card-art \{([\s\S]*?)\n\}/)?.[1] ?? "";
  assert.doesNotMatch(cardArt, /radial-gradient/);
  assert.doesNotMatch(css, /\.game-card\.rarity-uncommon \{[^}]*#d8e7c2/);
  assert.match(css, /\.game-card\.rarity-uncommon \{ background: #e8ddc4/);
});
...
test("经济过渡牌覆盖吃牌、弃牌、餐盘余量和商店折扣四种资源轴", () => {
  ...
  assert.equal(state.round.pending_gold_bonus, 2);
  ...
  const reserveState = readyState();
  reserveState.round.reserve_count = 2;
  addItem(reserveState, "IT116");
  engine.recordAction(reserveState, "discard", instance("A001", "jar"));
  assert.equal(reserveState.round.pending_gold_bonus, 1);
});
...
test("真实商店决策可完成 15 轮，三类构筑在高目标下胜率接近", () => {
  ...
  const reshuffleRun = simulateRun({ seed: 10, policy: "small" });
  assert.equal(reshuffleRun.won, true);

  const batch = simulateBatch({ seeds: 80 });
  const rates = batch.map((entry) => entry.win_rate);
  assert.ok(rates.every((rate) => rate >= 0.12 && rate <= 0.45), JSON.stringify(batch));
  assert.ok(Math.max(...rates) - Math.min(...rates) <= 0.12, JSON.stringify(batch));
});

test("平衡后的阶段目标提高为 120 / 750 / 4000", () => {
  assert.deepEqual(GAME_CONFIG.milestone_targets, { 5: 120, 10: 750, 15: 4000 });
});
