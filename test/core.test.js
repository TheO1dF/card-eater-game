...
test("商店道具包含可负担的基础池，核心道具提价并延后解锁", () => {
  const pool = createShopItemPool();
  const basic = pool.filter((entry) => entry.id >= "IT117" && entry.id <= "IT124");
  assert.equal(basic.length, 8);
  assert.ok(basic.every((entry) => entry.shop_price >= 3 && entry.shop_price <= 5));
  const reshuffleItem = pool.find((entry) => entry.id === "IT115");
  const membership = pool.find((entry) => entry.id === "IT113");
  assert.deepEqual([reshuffleItem.shop_price, reshuffleItem.min_shop_round], [16, 7]);
  assert.deepEqual([membership.shop_price, membership.min_shop_round], [12, 6]);
});
...
test("基础道具分别支持小牌组双启动、反食性与单类别构筑", () => {
  ...
  assert.equal(third.item_bonus, 1);
